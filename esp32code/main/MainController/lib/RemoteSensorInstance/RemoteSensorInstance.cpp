...
void RemoteSensorInstance::processPacket(String packet) {
    if (packet.startsWith("L:")) {
        _lastGPS = packet; // Save "L:lat,lon"
        _hasNewGPS = true; // Set flag so Main loop knows to send it
        return; // Done
    }
...
String RemoteSensorInstance::getGPS() { 
    _hasNewGPS = false;
    return _lastGPS; 
}
bool RemoteSensorInstance::hasGPS() { 
    return _hasNewGPS; 
}